...
void ASParNewGeneration::resize_spaces(size_t requested_eden_size,
  ...
  // We require eden and to space to be empty 调整时两者需为空
  if ((!eden()->is_empty()) || (!to()->is_empty())) {
    return;
  }

  size_t cur_eden_size = eden()->capacity();//eden空间当前容量

  if (PrintAdaptiveSizePolicy && Verbose) {// Verbose 为true 显示详细信息
  ...
  const size_t alignment = os::vm_page_size();
  const bool maintain_minimum =    //变更后的容量与新生代的最小容量比较
  ...
  if (from_start < to_start) {
    ...
    // and may not be attainable（可达）.  "eden_size" below is
    ...
    eden_size = align_size_down(eden_size, alignment);//计算eden_size 以alignment为倍数的下界数
    ...
    // Does the optimal to-space overlap(重叠) from-space?
    if (to_start < (char*)from()->end()) { // to-space和from-space区域有重叠部分
